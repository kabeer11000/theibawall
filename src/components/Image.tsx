...
const Image = (props: {
  ...
  // const [loaded, setLoaded] = useState<boolean>(false);
   const srcSet = [
    `${src.thumbnail.url} ${src.thumbnail.sizes[0]}w`,
    `${src.small.url} ${src.small.sizes[0]}w`,
    `${src.medium.url} ${src.medium.sizes[0]}w`,
    `${src.large.url} ${src.large.sizes[0]}w`,
  ].join(', ');
  return (
    <div
      key={index}
    ...
    >
      ...
      <div>
        ...
        <div className="z-0- -overflow-hidden md:lg:group-hover:z-[50]- grid grid-cols-1 -md:lg:group-hover:scale-125 transition-all duration-300">
          ...
          <NextImage
            {...props} src={src.large.url}
            width={src.large.sizes[0]} blurDataURL={src.thumbnail.url}
            height={src.large.sizes[1]} loading="lazy"
            alt={`Image ${index + 1}`} placeholder="blur" srcSet={srcSet} // Add responsive srcset
            sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" // Optional sizes attribute

            className="w-full h-auto md:lg:group-hover:brightness-50 md:lg:group-hover:outline outline-neutral-200 bg-neutral-100 rounded-md object-cover md:lg:group-hover:rounded-2xl transition-all duration-300"
          />
          <div className="flex mt-4 pointer-events-none justify-between">
