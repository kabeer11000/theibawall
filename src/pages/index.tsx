...
export default function Home() {
  ...
  return (
    <>
      ...
      <div style={{ background: 'rgba(255,255,255,1)' }} className='shadow- z-20 backdrop-blur-sm sticky top-0'>
      ...
      <p className='max-w-sm- md:lg:max-w-2xl px-4 block md:lg:hidden container mb-4 w-full mx-auto text-neutral-600'>
        Display <a href='#' className='font-bold underline' onClick={() => setMobileGrid(!mobileGrid)}>{mobileGrid ? "Feed" : "Grid"}</a>
      </p>

      <div className={`overflow-y-scroll md:lg:overflow-hidden min-h-[100vh] relative p-2 ${mobileGrid ? 'columns-1' : 'columns-2'} gap-y-8 pt-4 md:columns-4 lg:columns-5 xl:columns-6 gap-2 mb-[5rem] md:lg:p-2 md:lg:gap-2`}>
        {images.map((src, index) => (
          <Image key={src} src={src} id={'kabeerjaffri'} index={index} />
        ))}
